...
#pragma warning restore CS8600 // Se va a convertir un literal nulo o un posible valor nulo en un tipo que no acepta valores NULL
        ...
        private async void BajaCarrera()
        {
                string url = "http://localhost:5241/tecnicaturas/darBaja";
                string presupuestoJson = JsonConvert.SerializeObject(lTecnicatura[lstTecnicaturas.SelectedIndex]);
                ...
                foreach (Tecnicatura c in lTecnicatura)
                {
                    if (c.Id == lTecnicatura[lstTecnicaturas.SelectedIndex].Id)
                    {
                        try
                        {
                            ...
                            string presupuestoJson = JsonConvert.SerializeObject(c);
                            var data = await ClienteSingleton.GetInstance().PutAsync(url, presupuestoJson);
